...
require_once ("./27_Pager.class2.php");
//2.分页参数
$pagesize = 5;  //每页显示的数量
$curPage = isset($_GET['curPage']) ? $_GET['curPage']:1; //获取页码数，如果有则获取当前数，没有则为1
$startrow = ($curPage-1)*$pagesize;        //开始的行号
//3.获取总记录数和总页数
$sql = "SELECT * FROM user";
$records = $db ->rowCount($sql);
$totalPage = ceil($records/$pagesize);      //向上取整获取总页数
//4.获取分页数据
